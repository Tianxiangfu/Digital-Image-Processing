...
def create_mask_from_points(points, img_h, img_w):
    ...
    # Convert points to the correct format for OpenCV
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    for i in range(foreground_img.shape[1]):  # 遍历通道数
        foreground_channel = foreground_img[:, i:i+1, :, :]  # 取出第 i 个通道
        blended_channel = blended_img[:, i:i+1, :, :]        # 取出第 i 个通道

        # 计算前景和混合图像的 Laplacian
        foreground_laplacian = torch.nn.functional.conv2d(foreground_channel, laplacian_filter, padding=1)
        blended_laplacian = torch.nn.functional.conv2d(blended_channel, laplacian_filter, padding=1)

        # 计算 mask 里面的 L2 损失
        mask = foreground_mask * background_mask
        loss += torch.mean(mask * (foreground_laplacian - blended_laplacian) ** 2)
